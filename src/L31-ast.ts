...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | ClassExp; // TODO
...
//L31
export const makeClassExp = (fileds: VarDecl[], methods: Binding[]): ClassExp => ({             //TODO
    tag: "ClassExp",
    fileds: fileds,
    methods: methods
});
...
//L31
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp";              //TODO
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x)   || isClassExp(x);       //TODO
...
const parseL31GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
    keyword === "L31" && !isEmpty(body)
        ? bind(mapResult(parseL31Exp, body), (exps: Exp[]) =>
...
export const parseL31SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    isEmpty(params)
        ...
        : op === "class"
        ? parseClassExp(first(params), second(params))                 //TODO
        : makeFailure("Never");
...
const parseGoodDefine = (variable: Sexp, val: Sexp): Result<DefineExp> =>
    !isIdentifier(variable)
        ...
        : bind(parseL31CExp(val), (value: CExp) =>
              makeOk(makeDefineExp(makeVarDecl(variable), value))
          );
...
const parseAppExp = (op: Sexp, params: Sexp[]): Result<AppExp> =>
    safe2((rator: CExp, rands: CExp[]) => makeOk(makeAppExp(rator, rands)))(
        parseL31CExp(op),
        mapResult(parseL31CExp, params)
    );

const parseIfExp = (params: Sexp[]): Result<IfExp> =>
    params.length !== 3
        ? makeFailure("Expression not of the form (if <cexp> <cexp> <cexp>)")
        : bind(mapResult(parseL31CExp, params), (cexps: CExp[]) =>
              makeOk(makeIfExp(cexps[0], cexps[1], cexps[2]))
          );

const parseClassExp = (fileds: Sexp, methods: Sexp): Result<ClassExp> =>
    {    
        const lst = isArray(fileds) && allT(isString, fileds) ? map(makeVarDecl, fileds) : []
        const parsed_fields = makeOk(lst)
        ...
        const valsResult = mapResult(
            (binding) => parseL31CExp(second(binding)),
            methods
        );
        ...
        return safe2((vars: VarDecl[], bindings: Binding[]) => makeOk(makeClassExp(vars,bindings)))(parsed_fields, bindingsResult)
}
    
//export const makeClassExp = (fileds: VarDecl[], methods: Binding[]): ClassExp => ({     
// export const makeLetExp = (bindings: Binding[], body: CExp[]): LetExp => ({
// export const makeProcExp = (args: VarDecl[], body: CExp[]): ProcExp => ({

const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> =>
    isArray(vars) && allT(isString, vars)
        ? bind(mapResult(parseL31CExp, body), (cexps: CExp[]) =>
...
const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> => {
    ...
    return safe2(
        (bindings: Binding[], body: CExp[]) =>  makeOk(makeLetExp(bindings, body))
    )
...
const unparseLExps = (les: Exp[]): string => map(unparseL31, les).join(" ");
...
const unparseLetExp = (le: LetExp): string =>
    `(let (${map(
        (b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`,
        le.bindings
    ).join(" ")}) ${unparseLExps(le.body)})`;


const unparseClassExp = (cl: ClassExp): string =>
`(class (${map((v:VarDecl)=> v.var , cl.fileds).join(" ")}) (${map((b: Binding)=> `(${b.var.var} ${unparseL31(b.val)})` , cl.methods).join(" ")}))`;
...
export const unparseL31 = (exp: Program | Exp): string =>
    isBoolExp(exp)
        ...
        ? unparseProcExp(exp)
        : isIfExp(exp)
        ? `(if ${unparseL31(exp.test)} ${unparseL31(exp.then)} ${unparseL31(
              exp.alt
          )})`
        : isAppExp(exp)
        ...
        ? unparseLetExp(exp)
        : isDefineExp(exp)
        ? `(define ${exp.var.var} ${unparseL31(exp.val)})`
        : isProgram(exp)
